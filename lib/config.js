...
const remoteImageCdn = process.env.NEXT_PUBLIC_IMAGE_CDN
  ? process.env.NEXT_PUBLIC_IMAGE_CDN.replace(/\/+$/g, '')
  : ''

const LOCAL_PUBLIC_ASSET_PREFIXES = ['/images/', '/avatar.', '/bg_image.']
...
export const publicAssetUrl = value => {
  ...
  if (!remoteImageCdn || !value.startsWith('/')) {
    return value
  }

  const [pathname, query = ''] = value.split('?')
  const isLocalPublicAsset = LOCAL_PUBLIC_ASSET_PREFIXES.some(prefix =>
  ...
  const rewrittenPath = pathname.replace(/\.(png|jpe?g|gif|tiff?)$/i, '.webp')
  return `${remoteImageCdn}${rewrittenPath}${query ? `?${query}` : ''}`
}
...
export const convertVal = val => {
  ...
  const cdnAsset = publicAssetUrl(val)
  if (cdnAsset !== val) {
    return cdnAsset
  }
