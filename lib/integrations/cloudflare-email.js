...
function hasCloudflareEmailConfig(config = getEmailConfig()) {
  return Boolean(
    config.accountId && config.apiToken && config.fromEmail && config.replyTo
  )
}
...
function isInquiryLead(lead) {
  return /inquiry|recharge|chongzhi|claudecode|gpt/i.test(
    [lead.source, lead.product, lead.service].filter(Boolean).join(' ')
  )
}
...
function buildLeadOwnerEmail({ lead }) {
  ...
  const inquiryLead = isInquiryLead(lead)
  const details = [
    ['Email', lead.email],
    ['Source', source],
    ['Product', lead.product || 'not provided'],
  ...
  return {
    ...
    subject: inquiryLead
      ? `New CharliiAI inquiry: ${lead.email}`
      : `New CharliiAI lead: ${lead.email}`,
    html: buildEmailShell({
      eyebrow: inquiryLead ? 'Inquiry Alert' : 'Lead Alert',
      title: inquiryLead
        ? 'A new CharliiAI recharge inquiry just came in'
        : 'A new CharliiAI CTA lead just came in',
      bodyHtml: `
        <p style="margin:0 0 16px;font-size:15px;line-height:1.75;color:#334155;">
          ${
            inquiryLead
              ? 'The recharge inquiry form collected a new request. Key details are below.'
...
function buildLeadUserConfirmationEmail({ lead }) {
  ...
  return {
    ...
    subject: inquiryLead
      ? isZh
        ? '已收到你的咨询 | CharliiAI'
        : 'We received your inquiry | CharliiAI'
      : isZh
        ? '已收到你的订阅申请 | CharliiAI'
    ...
    html: buildEmailShell({
      ...
      title: inquiryLead
        ? isZh
          ? '已收到你的 CharliiAI 咨询'
          : 'Your CharliiAI inquiry was received'
        : isZh
          ? '你已经加入 CharliiAI 更新名单'
          : 'You are now on the CharliiAI update list',
      bodyHtml: inquiryLead
        ? isZh
          ? `
          <p style="margin:0 0 16px;font-size:15px;line-height:1.8;color:#334155;">
            已收到你的咨询邮箱：<strong>${escapeHtml(lead.email)}</strong>。
          </p>
          <p style="margin:0 0 16px;font-size:15px;line-height:1.8;color:#334155;">
            我会根据你留下的需求、联系方式和当前可用渠道尽快回复。
          </p>
          <div style="margin:20px 0;padding:18px;border-radius:18px;background:#f8fafc;border:1px solid #e2e8f0;">
            <div style="font-size:13px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:#0891b2;">咨询内容</div>
            <ul style="margin:12px 0 0 18px;padding:0;color:#334155;line-height:1.8;">
              <li>服务：${escapeHtml(lead.service || lead.product || '未填写')}</li>
              <li>偏好联系方式：${escapeHtml(lead.contactMethod || '未填写')}</li>
            </ul>
          </div>
          <div style="margin-top:20px;">
            <a href="${escapeHtml(
        ...
        `
          : `
          <p style="margin:0 0 16px;font-size:15px;line-height:1.8;color:#334155;">
            I received your inquiry email: <strong>${escapeHtml(lead.email)}</strong>.
          </p>
          <p style="margin:0 0 16px;font-size:15px;line-height:1.8;color:#334155;">
            I will follow up based on your request details, preferred contact method, and current availability.
          </p>
          <div style="margin:20px 0;padding:18px;border-radius:18px;background:#f8fafc;border:1px solid #e2e8f0;">
            <div style="font-size:13px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:#0891b2;">Inquiry details</div>
            <ul style="margin:12px 0 0 18px;padding:0;color:#334155;line-height:1.8;">
              <li>Service: ${escapeHtml(lead.service || lead.product || 'Not provided')}</li>
              <li>Preferred contact: ${escapeHtml(lead.contactMethod || 'Not provided')}</li>
            </ul>
          </div>
          <div style="margin-top:20px;">
            <a href="${escapeHtml(
    ...
    text: inquiryLead
      ? isZh
        ? `已收到你的 CharliiAI 咨询。\n邮箱：${lead.email}\n服务：${lead.service || lead.product || '未填写'}\n偏好联系方式：${lead.contactMethod || '未填写'}\n联系页：${contactUrl}`
        : `Your CharliiAI inquiry was received.\nEmail: ${lead.email}\nService: ${lead.service || lead.product || 'Not provided'}\nPreferred contact: ${lead.contactMethod || 'Not provided'}\nContact: ${contactUrl}`
      : isZh
