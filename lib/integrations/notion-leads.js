...
async function notionRequest(path, init = {}) {
  ...
  const response = await fetch(`${NOTION_API_BASE}${path}`, {
    ...
    headers: {
      ...
      'Notion-Version':
        normalizeEnv(process.env.NOTION_API_VERSION) || DEFAULT_NOTION_VERSION,
...
function buildLeadProperties(schema, lead) {
  ...
  const pageUrlEntry = findPreferredProperty(schema, [
    ...
    (name, property) =>
      property.type === 'url' && /^page[-_ ]?url$/i.test(name),
    (name, property) =>
    ...
    (name, property) => property.type === 'rich_text' && /url|link/i.test(name)
  ])
...
export { getLeadDatabaseConfig, hasLeadDatabaseConfig, storeLeadInNotion }
