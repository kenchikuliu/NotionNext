...
import { publicAssetUrl, siteConfig } from '../config'
    ...
    allPages: [
      {
        ...
        pageCoverThumbnail: publicAssetUrl(
          BLOG.HOME_BANNER_IMAGE || '/bg_image.jpg'
        ),
        date: {
...
function getSiteInfo({ collection, block, NOTION_CONFIG }) {
  ...
  const defaultPageCover = publicAssetUrl(
    NOTION_CONFIG?.HOME_BANNER_IMAGE || '/bg_image.jpg'
  )
  const defaultIcon = publicAssetUrl(NOTION_CONFIG?.AVATAR || '/avatar.svg')
