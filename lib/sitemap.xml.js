...
const CANONICAL_SLUG_REDIRECTS = new Map([
...
function getCanonicalSlug(rawSlug) {
  const normalizedSlug = normalizeSlug(rawSlug)
  return CANONICAL_SLUG_REDIRECTS.get(normalizedSlug.toLowerCase()) || normalizedSlug
}
...
export function generateSitemapXml({ allPages, NOTION_CONFIG }) {
  ...
  allPages?.forEach(post => {
    ...
    const slugWithoutLeadingSlash = getCanonicalSlug(post?.slug)
    urls.push({
